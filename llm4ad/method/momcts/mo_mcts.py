...
class MCTSNode:
    def __init__(self, algorithm, code, obj: List[float], depth=0, is_root=False, parent=None, visit=0, raw_info=None,
        ...
        self.reward_vector: List[float] = -1 * np.array(
            obj)  # a list, suppose all obj should be minimized so reward_vector should be maximize
...
class MCTS:
    def __init__(self, root_answer, alpha, lambad0, num_objectives: int):
        self.exploration_constant_0 = 0.1  # Paramter \lambda_0
        self.alpha = alpha
        self.lambad0 = lambad0 # Paramter \alpha
        self.max_depth = 10
        self.epsilon = 1e-10
        ...
        self.root = MCTSNode(algorithm=root_answer, code=root_answer, depth=0, obj=[0.0] * num_objectives, is_root=True,
                             Q_vector=[0.0] * num_objectives)
    ...
    def backpropagate(self, node: MCTSNode):
        ...
        while parent:  # how does parents update their Q
            ...
            for i in range(self.num_objectives):
                parent.Q_vector[i] = parent.Q_vector[i] + (node.reward_vector[i] - parent.Q_vector[i]) / (
                            parent.visits + 1)  # incremental average
    ...
    def pareto_best_child(self, node: MCTSNode) -> MCTSNode:
        ...
        if pareto_optimal_set:
            best_child = random.choice(
                pareto_optimal_set)  # (among a list of non-dominated solutions, randomly choose one)
            return best_child
        else:
    ...
    def pareto_ucb(self, node: MCTSNode, parent_visits: int) -> List[float]:
        ...
        ucb_vector = []
        for i in range(self.num_objectives):
            exploration_term = math.sqrt((4 * math.log(parent_visits) + math.log(self.num_objectives)) / (
                        2 * node.visits)) if node.visits > 0 else float('inf')
